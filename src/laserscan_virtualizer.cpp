...
#include <slip_ira_laser_tools/laserscan_virtualizerConfig.h>
#include <string.h>
...
void LaserscanVirtualizer::virtual_laser_scan_parser()
{
  ...
  for (int i = 0; i < tokens.size(); i++)
  {
    ...
    if (tfListener_.waitForTransform(
            ...
            tokens[i],
            ros::Time(0),
            ros::Duration(1.0))) // Check if TF knows the transform from this frame reference to
  ...
  std::vector<string>::iterator last =
      std::unique(tmp_output_frames.begin(), tmp_output_frames.end());
      ...
      !equal(tmp_output_frames.begin(), tmp_output_frames.end(), output_frames.begin()))
  {
    ...
    if (output_frames.size() > 0)
    {
      ...
      for (int i = 0; i < output_frames.size(); ++i)
      {
        ...
        {
          virtual_scan_publishers[i] =
              node_.advertise<sensor_msgs::LaserScan>(output_frames[i].c_str(), 1);
          cout << "\t\t" << output_frames[i] << " on topic " << output_frames[i].c_str() << endl;
        }
        else
        {
          virtual_scan_publishers[i] =
              node_.advertise<sensor_msgs::LaserScan>(output_laser_topic.c_str(), 1);
...
void LaserscanVirtualizer::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& pcl_in)
{
  if (cloud_frame.empty())
  {
    ...
    for (int i = 0; i < output_frames.size(); i++)
      if (tfListener_.waitForTransform(
              output_frames[i], cloud_frame, ros::Time(0), ros::Duration(2.0)))
  ...
  for (int i = 0; i < output_frames.size(); i++)
  {
    ...
    // Initialize the header of the temporary pointcloud, needed to rototranslate the three points
    // that define our plane It shall be equal to the input point cloud's one, changing only the
    // 'frame_id'
    ...
    // Extract the points close to the z=0 plane, convert them into a laser-scan message and publish
    // it
